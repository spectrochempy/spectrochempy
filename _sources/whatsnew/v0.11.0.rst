...
These are the changes in SpectroChemPy-0.11.0.
...
- Plotting behavior is more consistent again: scatter plots now show markers as
  ...
  consistently, the experimental Plotly backend has been removed (``use_plotly``
  is no longer available), and legacy ``lines`` / ``pen`` aliases continue to work across
