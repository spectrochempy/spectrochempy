...
from traitlets import (Bool, Unicode, Tuple, List, Integer, Float, Enum, observe, All, default, TraitError, Union,
                       Undefined, Set)
...
class MatplotlibPreferences(MetaConfigurable):
    ...
    markers_fillstyle = Enum('full|left|right|bottom|top|none'.split('|'), default_value='full',
                             help=r'''full|left|right|bottom|top|none''').tag(config=True, kind='')
    ...
    patch_linewidth = Float(0.3, help=r'''edge width in points.''').tag(config=True, kind='')
    patch_facecolor = Unicode('4C72B0', help=r'''''').tag(config=True, kind='color')
    patch_edgecolor = Unicode('black', help=r'''if forced, or patch is not filled''').tag(config=True, kind='color')
    patch_force_edgecolor = Bool(False, help=r'''True to always use edgecolor''').tag(config=True, kind='')
    patch_antialiased = Bool(True, help=r'''render patches in antialiased (no jaggies)''').tag(config=True, kind='')
    ...
    #
    font_family = Enum(['sans-serif', 'serif', 'cursive', 'monospace', 'fantasy'], default_value='sans-serif',
                       help=r'''sans-serif|serif|cursive|monospace|fantasy''').tag(config=True, kind='')
    font_style = Enum(['normal', 'roman', 'italic', 'oblique'], default_value='normal',
                      help=r'''normal (or roman), italic or oblique''').tag(config=True, kind='')
    font_variant = Enum(['normal', 'small-caps'], default_value='normal', help=r'''''').tag(config=True, kind='')
    ...
    text_usetex = Bool(False, help=r'''use latex for all text handling. The following fonts 
                       ...
                       at the matplotlib mailing list''').tag(config=True, kind='')
    latex_preamble = Unicode(r"""\usepackage{siunitx}
                            ...
                            """, help=r'''Latex preamble for matplotlib outputs
                             ...
                             may also be loaded, depending on your font settings.''').tag(config=True, kind='')

    text_hinting = Enum(['none', 'auto', 'native', 'either'], default_value='auto', help=r'''May be one of the 
    following: 'none': Perform no hinting
                         ...
                         * 'either': Use the native hinting information or the autohinter if none is available.
                         ...
                         'none'.''').tag(config=True)
    text_hinting_factor = Float(8, help=r'''Specifies the amount of softness for hinting in the horizontal 
    direction. A value of 1 will hint to full pixels. A value of 2 will hint to half pixels etc.''').tag(config=True,
                                                                                                         kind='')

    text_antialiased = Bool(True, help=r'''If True (default), the text will be antialiased. 
                            This only affects the Agg backend.''').tag(config=True, kind='')
    ...
    image_composite_image = Bool(True, help=r'''When True, all the images on a set of axes are
    ...
    is the 
                          amplitude of the wiggle perpendicular to the line (in pixels). *length* is the length of 
    ...
    def __init__(self, **kwargs):
        ...
        for key in plt.rcParams:
            l = key.split('.')
            if len(l) > 1:
                self._groups.add(l.pop(0))
            if len(l) > 1:
                self.subgroups.add(l.pop(0))
            if len(l) > 1:
                raise NotImplementedError
            self._members.add(l[0])
    ...
    def to_rc_key(self, key):
        rckey = ""
        l = key.split('_')
        if len(l)>1 and l[0] in self.groups:
            rckey += l.pop(0)
            rckey += '.'
        if len(l)>1 and l[0] in self.subgroups:
            rckey += l.pop(0)
            rckey += '.'
        rckey += '_'.join(l)
        return rckey
    ...
    def _anytrait_changed(self, change):
        ...
        if change.name in self.trait_names(config=True):
            ...
            # @observe('use_latex')  # def _use_latex_changed(self, change):  #     mpl.rc('text', usetex=change.new)
            #  # @observe('latex_preamble')  # def _set_latex_preamble(self, change):  #     mpl.rcParams[  #    #  #
            #  'text.latex.preamble'] = change.new.split('\n')
