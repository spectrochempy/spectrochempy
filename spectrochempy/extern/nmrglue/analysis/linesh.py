...
def pack_table(pbest, abest, iers, rec, param_columns, amp_column,
    ...
    # pack the parameters
    for dbest, dcolumns in zip(list(zip(*pbest)), param_columns):
        for p, c in zip(list(zip(*dbest)), dcolumns):
            rec[c] = p
...
def unpack_table(rec, param_columns, amp_column):
    ...
    """
    params = list(zip(*[list(zip(*[rec[c] for c in dc])) for dc in param_columns]))
    amps = rec[amp_column]
    return params, amps
