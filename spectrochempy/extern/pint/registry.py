...
class BaseRegistry(meta.with_metaclass(_Meta)):
    ...
    def _build_cache(self):
        ...
        deps = dict((name, set(list(definition.reference.keys()) if definition.reference else {}))
    ...
    def _get_dimensionality(self, input_units):
        ...
        dims = UnitsContainer(dict((k, v) for k, v in list(accumulator.items())
                                   if v != 0.0))
    ...
    def _get_dimensionality_ratio(self, unit1, unit2):
        ...
        if not dim1 or not dim2 or list(dim1.keys()) != list(dim2.keys()): #not comparable
            return None

        ratios = (dim2[key]/val for key, val in list(dim1.items()))
        first = next(ratios)
    ...
    def _get_root_units(self, input_units, check_nonmult=True):
        ...
        units = UnitsContainer(dict((k, v) for k, v in list(accumulators[1].items())
                                    if v != 0.))
...
class NonMultiplicativeRegistry(BaseRegistry):
    ...
    def _validate_and_extract(self, units):
        ...
        nonmult_units = [(u, e) for u, e in list(units.items())
...
class SystemRegistry(BaseRegistry):
    ...
    def _get_base_units(self, input_units, check_nonmult=True, system=None):
        ...
        for unit, value in list(units.items()):
            if unit in bu:
                new_unit = bu[unit]
