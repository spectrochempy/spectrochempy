...
from spectrochempy.extern.pint import DimensionalityError, set_application_registry
from spectrochempy.extern.pint.compat import np
from spectrochempy.extern.pint.testsuite import QuantityTestCase, helpers
from spectrochempy.extern.pint.testsuite.test_umath import TestUFuncs
...
class TestNDArrayQunatityMath(QuantityTestCase):
    ...
    def test_exponentiation_array_exp(self):
        arr = np.array(list(range(3)), dtype=np.float)
        q = self.Q_(arr, 'meter')
