...
from spectrochempy.extern.pint.compat import babel_units, Loc, string_types
...
def format_unit(unit, spec, **kwspec):
    ...
    if spec == 'L':
        rm = [(r'\mathrm{{{}}}'.format(u), p) for u, p in list(unit.items())]
        result = formatter(rm, **fmt)
    else:
        result = formatter(list(unit.items()), **fmt)
    if spec == 'L':
...
def siunitx_format_unit(units):
    ...
    for unit, power in sorted(units._units.items()):
        ...
        prefix = None
        for p in list(registry._prefixes.values()):
            p = str(p)
