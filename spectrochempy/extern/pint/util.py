...
def solve_dependencies(dependencies):
    ...
    while d:
        ...
        if not t:
            raise ValueError('Cyclic dependencies exist among these items: {}'.format(', '.join(repr(x) for x in list(d.items()))))
        r.append(t)
        # and cleaned up
...
class UnitsContainer(Mapping):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for key, value in list(d.items()):
            ...
            if not isinstance(value, float):
                d[key] = float(value)
        self._hash = hash(frozenset(list(self._d.items())))
    ...
    def __mul__(self, other):
        ...
        for key, value in list(other.items()):
            d[key] += value
        keys = [key for key, value in list(d.items()) if value == 0]
        for key in keys:
            del d[key]
...
class ParserHelper(UnitsContainer):
    ...
    def __mul__(self, other):
        if isinstance(other, string_types):
            ...
            new = self.operate(list(other.items()))
    ...
    def __truediv__(self, other):
        if isinstance(other, string_types):
            ...
            new = self.operate(list(other.items()), operator.sub)
            new.scale /= other.scale
        else:
            new = self.operate(list(other.items()), operator.sub)
        return new
    ...
    def __rtruediv__(self, other):
        ...
        if isinstance(other, string_types):
            ...
            new = self.operate(list(other.items()), operator.add)
            new.scale *= other.scale
        else:
            new = new.operate(list(other.items()), operator.add)
        return new
...
def infer_base_unit(q):
    ...
    for unit_name, power in list(q._units.items()):
        completely_parsed_unit = list(parse(unit_name))[-1]

        _, base_unit, __ = completely_parsed_unit
        d[base_unit] += power
    return UnitsContainer(dict((k, v) for k, v in list(d.items()) if v != 0))  # remove values that resulted in a power of 0
