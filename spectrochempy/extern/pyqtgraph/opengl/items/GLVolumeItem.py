...
class GLVolumeItem(GLGraphicsItem) :
    ...
    **Bases:** :class:`GLGraphicsItem <pyqtgraph.opengl.GLGraphicsItem>`
    ...
    def __init__(self, data, sliceDensity=1, smooth=True,
        ...
        =======================================================================================
        **Arguments:**
        data            Volume data to be rendered. *Must* be 4D numpy array
        (x, y, z, RGBA) with dtype=ubyte.
        sliceDensity    Density of slices to render through the volume. A
        value of 1 means one slice per voxel.
        smooth          (bool) If True, the volume slices are rendered with
    ...
    def _uploadData(self) :
        ...
        glTexImage3D(GL_PROXY_TEXTURE_3D, 0, GL_RGBA, shape[0], shape[1],
                     shape[2], 0, GL_RGBA, GL_UNSIGNED_BYTE, None)
        if glGetTexLevelParameteriv(GL_PROXY_TEXTURE_3D, 0,
                                    GL_TEXTURE_WIDTH) == 0 :
            raise Exception(
                "OpenGL failed to create 3D texture (%dx%dx%d); too large "
                "for this hardware." % shape[
                                                                                                :3])

        glTexImage3D(GL_TEXTURE_3D, 0, GL_RGBA, shape[0], shape[1], shape[2],
    ...
    def paint(self) :
        ...
        # print "center", center, "cam", view.cameraPosition(),
        # self.mapFromParent(view.cameraPosition()), "diff", cam
