    ...
    **Bases:** :class:`GLGraphicsItem <pyqtgraph.opengl.GLGraphicsItem>`
    ...
    def __init__(self, **kwds) :
        ...
        **Arguments:**
        ...
        ============== =====================================================
        """
        self.opts = {'meshdata' : None, 'color' : (1., 1., 1., 1.),
            'drawEdges'         : False, 'drawFaces' : True,
            'edgeColor'         : (0.5, 0.5, 0.5, 1.0), 'shader' : None,
    ...
    def setShader(self, shader) :
        """Set the shader used when rendering faces in the mesh. (see the GL
        shaders example)"""
    ...
    def setColor(self, c) :
        """Set the default color to use when no vertex or face colors are
        specified."""
        self.opts['color'] = c
        self.update()
    ...
    def setMeshData(self, **kwds) :
        ...
        2. Specify keyword arguments to be passed to MeshData(..) to create
        a new instance.
        """
        md = kwds.get('meshdata', None)
        if md is None :
            opts = {}
            for k in ['vertexes', 'faces', 'edges', 'vertexColors',
                      'faceColors'] :
                try :
                    opts[k] = kwds.pop(k)
                except KeyError :
    ...
    def parseMeshData(self) :
        ...
        if self.vertexes is not None and self.normals is not None :
            ...
            # self.opts['meshdata'] = MeshData(vertexes=self.opts[
            # 'vertexes'], faces=self.opts['faces'])
