...
class GLScatterPlotItem(GLGraphicsItem) :
    ...
    def __init__(self, **kwds) :
        ...
        self.pxMode = True
        # self.vbo = {}      ## VBO does not appear to improve performance
        # very much.
        self.setData(**kwds)
    ...
    def setData(self, **kwds) :
        ...
        **Arguments:**
        ...
        pxMode                If True, spot sizes are expressed in pixels. 
                              Otherwise, they are expressed in item
                              coordinates.
        ====================
        ==================================================
        """
        ...
        for k in kwds.keys() :
            if k not in args :
                raise Exception(
                    'Invalid keyword argument: %s (allowed arguments are '
                    '%s)' % (
                    k, str(args)))
    ...
    def initializeGL(self) :
        ...
        # """Prepare OpenGL state for drawing. This function is called
        # immediately before painting."""
        ##glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA)  ## requires
        # z-sorting to render properly.
    ...
    def paint(self) :
        ...
        with self.shader :
            # glUniform1i(self.shader.uniform('texture'), 0)  ## inform the
            # shader which texture to use
            glEnableClientState(GL_VERTEX_ARRAY)
            try :
                pos = self.pos
                # if pos.ndim > 2:
                # pos = pos.reshape((reduce(lambda a,b: a*b, pos.shape[
                # :-1]), pos.shape[-1]))
                ...
                if not self.pxMode or isinstance(self.size, np.ndarray) :
                    ...
                    glNormal3f(self.size, 0, 0)  ## vertex shader uses norm
                    # .x to determine point size
