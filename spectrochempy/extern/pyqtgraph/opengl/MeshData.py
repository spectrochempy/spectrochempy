...
class MeshData(object) :
    ...
    list of faces]
    format (suitable for use with glDrawElements) and 'indexed' [list of
    vertexes] format
    (suitable for use with glDrawArrays). It will automatically compute face
    normal
    ...
    def __init__(self, vertexes=None, faces=None, edges=None,
        ...
        **Arguments:**
        ...
        self._vertexesIndexedByFaces = None  # (Nf, 3, 3) array of vertex
        # coordinates
        self._vertexesIndexedByEdges = None  # (Ne, 2, 3) array of vertex
        # coordinates

        ## mappings between vertexes, faces, and edges
        self._faces = None  # Nx3 array of indexes into self._vertexes
        # specifying three vertexes for each face
        self._edges = None  # Nx2 array of indexes into self._vertexes
        # specifying two vertexes per edge
        self._vertexFaces = None  ## maps vertex ID to a list of face IDs (
        # inverse mapping of _faces)
        self._vertexEdges = None  ## maps vertex ID to a list of edge IDs (
        # inverse mapping of _edges)
        ...
        self._faceNormalsIndexedByFaces = None  # (Nf, 3, 3) array of face
        # normals
        self._faceColors = None  # (Nf, 4) array of face colors
        self._faceColorsIndexedByFaces = None  # (Nf, 3, 4) array of face
        # colors
        self._faceColorsIndexedByEdges = None  # (Ne, 2, 4) array of face
        # colors

        ## Per-edge data
        self._edgeColors = None  # (Ne, 4) array of edge colors
        self._edgeColorsIndexedByEdges = None  # (Ne, 2, 4) array of edge
        # colors
    ...
    def faces(self) :
        ...
        If faces have not been computed for this mesh, the function returns
        None.
        """
        return self._faces

    def edges(self) :
        """Return an array (Nf, 3) of vertex indexes, two per edge in the
        mesh."""
    ...
    def vertexes(self, indexed=None) :
        ...
        vertexes
        per face in the mesh (and a single vertex may appear more than once
        in the array)."""
        if indexed is None :
            if self._vertexes is None and self._vertexesIndexedByFaces is \
                    not None :
                self._computeUnindexedVertexes()
            return self._vertexes
        elif indexed == 'faces' :
            if self._vertexesIndexedByFaces is None and self._vertexes is \
                    not None :
    ...
    def vertexNormals(self, indexed=None) :
        ...
        vertex in the mesh.
        If indexed is 'faces', then the array will contain three normal
        vectors per face
    ...
    def faceColors(self, indexed=None) :
        ...
        if indexed is None :
            ...
            if self._faceColorsIndexedByFaces is None and self._faceColors \
                    is not None :
                Nf = self._faceColors.shape[0]
                self._faceColorsIndexedByFaces = np.empty((Nf, 3, 4),
                                                          dtype=self._faceColors.dtype)
    ...
    def edgeColors(self) :
        ...
        # def _setIndexedFaces(self, faces, vertexColors=None,
        # faceColors=None):
    ...
    def _computeUnindexedVertexes(self) :
        ...
        for i in xrange(faces.shape[0]) :
            ...
            for j in range(face.shape[0]) :
                pt = face[j]
                pt2 = tuple([round(x * 1e14) for x in
                             pt])  ## quantize to be sure that
                # nearly-identical points will be merged
                ...
                if index is None :
                    ...
                    verts[pt2] = index
                self._vertexFaces[index].append(
                    i)  # keep track of which vertexes belong to which faces
    ...
    def sphere(rows, cols, radius=1.0, offset=True) :
        ...
        if offset :
            th = th + ((np.pi / cols) * np.arange(rows + 1).reshape(rows + 1,
                                                                    1))  ##
            # rotate each row by 1/2 column
        verts[..., 0] = s * np.cos(th)
        verts[..., 1] = s * np.sin(th)
        verts = verts.reshape((rows + 1) * cols, 3)[cols - 1 :-(
        cols - 1)]  ## remove redundant vertexes from top and bottom

        ## compute faces
        faces = np.empty((rows * cols * 2, 3), dtype=np.uint)
        rowtemplate1 = ((np.arange(cols).reshape(cols, 1) + np.array(
                [[0, 1, 0]])) % cols) + np.array([[0, 0, cols]])
        rowtemplate2 = ((np.arange(cols).reshape(cols, 1) + np.array(
                [[0, 1, 1]])) % cols) + np.array([[cols, 0, cols]])
        for row in range(rows) :
            start = row * cols * 2
            faces[start :start + cols] = rowtemplate1 + row * cols
            faces[start + cols :start + (cols * 2)] = rowtemplate2 + row * cols
        faces = faces[
                cols :-cols]  ## cut off zero-area triangles at top and bottom
    ...
    def cylinder(rows, cols, radius=[1.0, 1.0], length=1.0, offset=False) :
        ...
        verts = verts.reshape((rows + 1) * cols,
                              3)  # just reshape: no redundant vertices...
        ## compute faces
        faces = np.empty((rows * cols * 2, 3), dtype=np.uint)
        rowtemplate1 = ((np.arange(cols).reshape(cols, 1) + np.array(
                [[0, 1, 0]])) % cols) + np.array([[0, 0, cols]])
