...
class ScatterPlotWidget(QtGui.QSplitter) :
    ...
    def __init__(self, parent=None) :
        ...
        self.params = ptree.Parameter.create(name='params', type='group',
    ...
    def setFields(self, fields, mouseOverField=None) :
        ...
        :func:`ColorMapWidget.setFields
        <pyqtgraph.widgets.ColorMapWidget.ColorMapParameter.setFields>`
        """
    ...
    def updatePlot(self) :
        ...
        units = list([item.opts.get('units', '') for item in
                      self.fieldList.selectedItems()])
        if len(sel) == 0 :
            self.plot.setTitle('')
            return

        if len(sel) == 1 :
            self.plot.setLabels(left=('N', ''), bottom=(sel[0], units[0]),
                                title='')
            if len(data) == 0 :
                return
            # x = data[sel[0]]
            ...
            # for ax in [0,1]:
            # d = data[sel[ax]]
            ### scatter catecorical values just a bit so they show up better
            #  in the scatter plot.
        ...
        for i in [0, 1] :
            ...
            if xy[i] is not None and (
                    self.fields[sel[i]].get('mode', None) == 'enum' or xy[
                i].dtype.kind in ('S', 'O')) :
                vals = self.fields[sel[i]].get('values', list(set(xy[i])))
                xy[i] = np.array(
                        [vals.index(x) if x in vals else len(vals) for x in
                         xy[i]], dtype=float)
        ...
        if self.scatterPlot is not None :
            try :
                ...
                pass
        self.scatterPlot = self.plot.plot(xy[0], xy[1], data=data[mask],
                                          **style)
