...
class RemoteGraphicsView(QtGui.QWidget) :
    """
    Replacement for GraphicsView that does all scene management and
    rendering on a remote process,
    ...
    def __init__(self, parent=None, *args, **kwds) :
        """
        The keyword arguments 'useOpenGL' and 'backgound', if specified,
        are passed to the remote
        GraphicsView.__init__(). All other keyword arguments are passed to
        multiprocess.QtProcess.__init__().
        """
        self._img = None
        self._imgReq = None
        self._sizeHint = (640,
                          480)  ## no clue why this is needed, but it seems
        # to be the default sizeHint for GraphicsView.
        ## without it, the widget will not compete for space against another
        #  GraphicsView.
    ...
    def remoteSceneChanged(self, data) :
        ...
        if self.shm is None or self.shm.size != size :
            if self.shm is not None :
                self.shm.close()
            if sys.platform.startswith('win') :
                self.shmtag = newfile  ## on windows, we create a new tag
                # for every resize
                self.shm = mmap.mmap(-1, size,
                                     self.shmtag)  ## can't use tmpfile on
                # windows because the file can only be opened once.
        ...
        self._img.data = data  # data must be kept alive or PySide 1.2.1 (
        # and probably earlier) will crash.
        self.update()
    ...
    def paintEvent(self, ev) :
        ...
        p.drawImage(self.rect(), self._img,
                    QtCore.QRect(0, 0, self._img.width(), self._img.height()))
        p.end()

    def mousePressEvent(self, ev) :
        self._view.mousePressEvent(int(ev.type()), ev.pos(), ev.globalPos(),
                                   int(ev.button()), int(ev.buttons()),
                                   int(ev.modifiers()), _callSync='off')
        ev.accept()
        return QtGui.QWidget.mousePressEvent(self, ev)

    def mouseReleaseEvent(self, ev) :
        self._view.mouseReleaseEvent(int(ev.type()), ev.pos(), ev.globalPos(),
                                     int(ev.button()), int(ev.buttons()),
                                     int(ev.modifiers()), _callSync='off')
        ev.accept()
    ...
    def mouseMoveEvent(self, ev) :
        self._view.mouseMoveEvent(int(ev.type()), ev.pos(), ev.globalPos(),
                                  int(ev.button()), int(ev.buttons()),
                                  int(ev.modifiers()), _callSync='off')
        ev.accept()
    ...
    def wheelEvent(self, ev) :
        self._view.wheelEvent(ev.pos(), ev.globalPos(), ev.delta(),
                              int(ev.buttons()), int(ev.modifiers()),
                              int(ev.orientation()), _callSync='off')
        ev.accept()
        return QtGui.QWidget.wheelEvent(self, ev)

    def keyEvent(self, ev) :
        if self._view.keyEvent(int(ev.type()), int(ev.modifiers()), text,
                               autorep, count) :
            ev.accept()
    ...
    def remoteProcess(self) :
        """Return the remote process handle. (see
        multiprocess.remoteproxy.RemoteEventHandler)"""
        return self._proc

    def close(self) :
        """Close the remote process. After this call, the widget will no
        longer be updated."""
        self._proc.close()
...
class Renderer(GraphicsView) :
    ...
    def __init__(self, *args, **kwds) :
        ...
        if sys.platform.startswith('win') :
            self.shmtag = "pyqtgraph_shmem_" + ''.join(
                    [chr((random.getrandbits(20) % 25) + 97) for i in
                     range(20)])
            self.shm = mmap.mmap(-1, mmap.PAGESIZE,
                                 self.shmtag)  # use anonymous mmap on windows
        else :
            self.shmFile = tempfile.NamedTemporaryFile(
                prefix='pyqtgraph_shmem_')
            self.shmFile.write(b'\x00' * (mmap.PAGESIZE + 1))
            fd = self.shmFile.fileno()
            self.shm = mmap.mmap(fd, mmap.PAGESIZE, mmap.MAP_SHARED,
                                 mmap.PROT_WRITE)
    ...
    def renderView(self) :
        if self.img is None :
            ...
            if size > self.shm.size() :
                if sys.platform.startswith('win') :
                    ## windows says "WindowsError: [Error 87] the parameter
                    # is incorrect" if we try to resize the mmap
                    self.shm.close()
                    ## it also says (sometimes) 'access is denied' if we try
                    #  to reuse the tag.
                    self.shmtag = "pyqtgraph_shmem_" + ''.join(
                            [chr((random.getrandbits(20) % 25) + 97) for i in
                             range(20)])
    ...
    def mousePressEvent(self, typ, pos, gpos, btn, btns, mods) :
        ...
        return GraphicsView.mousePressEvent(self,
                                            QtGui.QMouseEvent(typ, pos, gpos,
                                                              btn, btns, mods))
    ...
    def mouseMoveEvent(self, typ, pos, gpos, btn, btns, mods) :
        ...
        btns = QtCore.Qt.MouseButtons(btns)
        mods = QtCore.Qt.KeyboardModifiers(mods)
        return GraphicsView.mouseMoveEvent(self,
                                           QtGui.QMouseEvent(typ, pos, gpos,
                                                             btn, btns, mods))
    ...
    def mouseReleaseEvent(self, typ, pos, gpos, btn, btns, mods) :
        ...
        btns = QtCore.Qt.MouseButtons(btns)
        mods = QtCore.Qt.KeyboardModifiers(mods)
        return GraphicsView.mouseReleaseEvent(self,
                                              QtGui.QMouseEvent(typ, pos, gpos,
                                                                btn, btns,
                                                                mods))
    ...
    def wheelEvent(self, pos, gpos, d, btns, mods, ori) :
        ...
        return GraphicsView.wheelEvent(self,
                                       QtGui.QWheelEvent(pos, gpos, d, btns,
                                                         mods, ori))

    def keyEvent(self, typ, mods, text, autorep, count) :
        typ = QtCore.QEvent.Type(typ)
        mods = QtCore.Qt.KeyboardModifiers(mods)
        GraphicsView.keyEvent(self,
                              QtGui.QKeyEvent(typ, mods, text, autorep, count))
        return ev.accepted()
