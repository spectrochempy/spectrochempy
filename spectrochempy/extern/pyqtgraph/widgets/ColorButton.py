...
class ColorButton(QtGui.QPushButton) :
    ...
    **Bases:** QtGui.QPushButton
    ...
    ============================================================
    **Signals:**
    sigColorChanging(self) emitted whenever a new color is picked in the
    color dialog
    sigColorChanged(self)  emitted when the selected color is accepted (user
    clicks OK)
    ======================
    ============================================================
    """
    sigColorChanging = QtCore.Signal(
        object)  ## emitted whenever a new color is picked in the color dialog
    sigColorChanged = QtCore.Signal(
        object)  ## emitted when the selected color is accepted (user clicks
    #  OK)
    ...
    def __init__(self, parent=None, color=(128, 128, 128)) :
        ...
        # QtCore.QObject.connect(self.colorDialog, QtCore.SIGNAL(
        # 'currentColorChanged(const QColor&)'), self.currentColorChanged)
        # QtCore.QObject.connect(self.colorDialog, QtCore.SIGNAL('rejected(
        # )'), self.currentColorRejected)
    ...
    def paintEvent(self, ev) :
        ...
        ## draw white base, then texture for indicating transparency,
        # then actual color
    ...
    def setColor(self, color, finished=True) :
        """Sets the button's color and emits both sigColorChanged and
        sigColorChanging."""
