...
class TreeWidget(QtGui.QTreeWidget) :
    """Extends QTreeWidget to allow internal drag/drop with widgets in the
    tree.
    Also maintains the expanded state of subtrees as they are moved.
    This class demonstrates the absurd lengths one must go to to make
    drag/drop work."""

    sigItemMoved = QtCore.Signal(object, object,
                                 object)  # (item, parent, index)
    ...
    def setItemWidget(self, item, col, wid) :
        """
        Overrides QTreeWidget.setItemWidget such that widgets are added
        inside an invisible wrapper widget.
        This makes it possible to move the item in and out of the tree
        without its widgets being automatically deleted.
        """
    ...
    def recoverMove(self, item) :
        ...
        item.setExpanded(
            False)  ## Items do not re-expand correctly unless they are
        # collapsed first.
    ...
    def updateDropFlags(self) :
        ...
        ### self.childNestingLimit is upheld when moving items without
        # children, but if the item being moved has children/grandchildren,
        # the children/grandchildren
        ### can end up over the childNestingLimit. 
        if self.childNestingLimit == None :
            pass  # enable drops in all items (but only if there are drops
            # that aren't enabled? for performance...)
        else :
            ...
            for item in items :
                ...
                if parentCount >= self.childNestingLimit :
                    item.setFlags(
                        item.flags() & (~QtCore.Qt.ItemIsDropEnabled))
                else :
...
class TreeWidgetItem(QtGui.QTreeWidgetItem) :
    ...
    def setChecked(self, column, checked) :
        self.setCheckState(column,
                           QtCore.Qt.Checked if checked else
                           QtCore.Qt.Unchecked)
