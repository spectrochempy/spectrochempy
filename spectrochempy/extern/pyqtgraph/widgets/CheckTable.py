...
class CheckTable(QtGui.QWidget) :
    ...
    def __init__(self, columns) :
        ...
        self.rowNames = []
        self.rowWidgets = []
        self.oldRows = {}  ## remember settings from removed rows; reapply
        # if they reappear.
    ...
    def addRow(self, name) :
        ...
        for c in self.columns :
            ...
            col += 1
            # QtCore.QObject.connect(check, QtCore.SIGNAL('stateChanged(
            # int)'), self.checkChanged)
    ...
    def removeRow(self, name) :
        ...
        for w in self.rowWidgets[row] :
            w.setParent(None)
            # QtCore.QObject.disconnect(w, QtCore.SIGNAL('stateChanged(
            # int)'), self.checkChanged)
    ...
    def saveState(self) :
        ...
        for i in range(len(self.rowNames)) :
            row = [self.rowNames[i]] + [c.isChecked() for c in
                                        self.rowWidgets[i][1 :]]
            rows.append(row)
