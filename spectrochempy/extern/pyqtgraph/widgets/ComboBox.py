...
class ComboBox(QtGui.QComboBox) :
    ...
    * Handles dict mappings -- user selects a text key, and the ComboBox
    indicates
      the selected value.
    ...
    * setItems() replaces the items in the ComboBox and blocks signals if the
    ...
    def __init__(self, parent=None, items=None, default=None) :
        ...
        # self.value = default
        if 'darwin' in sys.platform :  ## because MacOSX can show names that
            #  are wider than the comboBox
    ...
    def value(self) :
        ...
        corresponding to the currently selected key. If the combo list is
        empty,
        return None.
        """
        if self.count() == 0 :
    ...
    def addItem(self, *args, **kwds) :
        # Need to handle two different function signatures for
        # QComboBox.addItem
        try :
            ...
            raise TypeError(
                "First or second argument of addItem must be a string.")
    ...
    def addItems(self, items) :
        if isinstance(items, list) :
            ...
            raise TypeError(
                "items argument must be list or dict (got %s)." % type(items))
