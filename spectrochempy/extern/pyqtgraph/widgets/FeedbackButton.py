...
class FeedbackButton(QtGui.QPushButton) :
    """
    QPushButton which flashes success/failure indication for slow or
    ...
    def feedback(self, success, message=None, tip="", limitedTime=True) :
        """Calls success() or failure(). If you want the message to be
        displayed until the user takes an action, set limitedTime to False.
        Then call self.reset() after the desired action.Threadsafe."""
        if success :
    ...
    def success(self, message=None, tip="", limitedTime=True) :
        """Displays specified message on button and flashes button green to
        let user know action was successful. If you want the success to be
        displayed until the user takes an action, set limitedTime to False.
        Then call self.reset() after the desired action. Threadsafe."""
    ...
    def failure(self, message=None, tip="", limitedTime=True) :
        """Displays specified message on button and flashes button red to
        let user know there was an error. If you want the error to be
        displayed until the user takes an action, set limitedTime to False.
        Then call self.reset() after the desired action. Threadsafe. """
        isGuiThread = QtCore.QThread.currentThread() == \
                      QtCore.QCoreApplication.instance().thread()
    ...
    def processing(self, message="Processing..", tip="", processEvents=True) :
        """Displays specified message on button to let user know the action
        is in progress. Threadsafe. """
        isGuiThread = QtCore.QThread.currentThread() == \
                      QtCore.QCoreApplication.instance().thread()
        if isGuiThread :
    ...
    def startBlink(self, color, message=None, tip="", limitedTime=True) :
        ...
        self.setToolTip(tip, temporary=True)
        self.count = 0
        # self.indStyle = "QPushButton {border: 2px solid %s; border-radius:
        #  5px}" % color
...
if __name__ == '__main__' :
    ...
    def click() :
        ...
        fail = not fail
        if fail :
            btn.failure(message="FAIL.",
                        tip="There was a failure. Get over it.")
        else :
