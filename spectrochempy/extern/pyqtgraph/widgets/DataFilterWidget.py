...
class DataFilterParameter(ptree.types.GroupParameter) :
    ...
    def __init__(self) :
        self.fields = {}
        ptree.types.GroupParameter.__init__(self, name='Data Filter',
                                            addText='Add filter..', addList=[])
    ...
    def generateMask(self, data) :
        ...
        for fp in self :
            ...
            # vals = data[key]
            # mask &= (vals >= mn)
            # mask &= (vals < mx)  ## Use inclusive minimum and
            # non-inclusive maximum. This makes it easier to create
            # non-overlapping selections
        return mask

    def describe(self) :
        """Return a list of strings describing the currently enabled
        filters."""
        desc = []
        for fp in self :
            if fp.value() is False :
...
class RangeFilterItem(ptree.types.SimpleParameter) :
    def __init__(self, name, opts) :
        ...
        ptree.types.SimpleParameter.__init__(self, name=name,
                                             ...
                                             children=[
                                                 # dict(name="Field",
                                                 # type='list', value=name,
                                                 # values=fields),
                                                 dict(name='Min', type='float',
                                                      value=0.0, suffix=units,
                                                      siPrefix=True),
                                                 dict(name='Max', type='float',
                                                      value=1.0, suffix=units,
                                                      siPrefix=True), ])

    def generateMask(self, data, mask) :
        vals = data[self.fieldName][mask]
        mask[mask] = (vals >= self['Min']) & (vals < self[
            'Max'])  ## Use inclusive minimum and non-inclusive maximum.
        # This makes it easier to create non-overlapping selections
        return mask

    def describe(self) :
        return "%s < %s < %s" % (
        fn.siFormat(self['Min'], suffix=self.units), self.fieldName,
        fn.siFormat(self['Max'], suffix=self.units))
