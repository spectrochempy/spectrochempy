...
class GraphicsView(QtGui.QGraphicsView) :
    ...
    unambiguous control of the
    viewed coordinate range. Also automatically creates a GraphicsScene and
    a central QGraphicsWidget
    that is automatically scaled to the full view geometry.
    
    This widget is the basis for :class:`PlotWidget <pyqtgraph.PlotWidget>`, 
    :class:`GraphicsLayoutWidget <pyqtgraph.GraphicsLayoutWidget>`, and the
    view widget in
    :class:`ImageView <pyqtgraph.ImageView>`.
    
    By default, the view coordinate system matches the widget's pixel
    coordinates and
    automatically updates when the view is resized. This can be overridden
    by setting
    autoPixelRange=False. The exact visible range can be set with setRange().
    
    The view can be panned using the middle mouse button and scaled using
    the right mouse button if
    enabled via enableMouse()  (but ordinarily, we use ViewBox for this
    functionality)."""
    ...
    def __init__(self, parent=None, useOpenGL=None, background='default') :
        ...
        **Arguments:**
        ...
        useOpenGL       If True, the GraphicsView will use OpenGL to do all
        of its
                        rendering. This can improve performance on some
                        systems,
                        but may also introduce bugs (the combination of 
                        QGraphicsView and QGLWidget is still an 'experimental' 
                        feature of Qt)
        background      Set the background color of the GraphicsView.
        Accepts any
        ...
        self.scaleCenter = False  ## should scaling center around view
        # center (True) or mouse click (False)
        self.clickAccepted = False

    def setAntialiasing(self, aa) :
        """Enable or disable default antialiasing.
        Note that this will only affect items that do not specify their own
        antialiasing options."""
        if aa :
            self.setRenderHints(
                self.renderHints() | QtGui.QPainter.Antialiasing)
        else :
            self.setRenderHints(
                self.renderHints() & ~QtGui.QPainter.Antialiasing)
    ...
    def setBackground(self, background) :
        ...
        To use the defaults specified py pyqtgraph.setConfigOption,
    ...
    def useOpenGL(self, b=True) :
        if b :
            if not HAVE_OPENGL :
                raise Exception(
                    "Requested to use OpenGL with QGraphicsView, "
                    "but QtOpenGL module is not available.")
    ...
    def setCentralWidget(self, item) :
        """Sets a QGraphicsWidget to automatically fill the entire view (the
    ...
    def resizeEvent(self, ev) :
        ...
        if self.autoPixelRange :
            self.range = QtCore.QRectF(0, 0, self.size().width(),
                                       self.size().height())
        GraphicsView.setRange(self, self.range, padding=0,
                              disableAutoPixel=False)  ## we do this because
        #  some subclasses like to redefine setRange in an incompatible way.
        self.updateMatrix()
    ...
    def setRange(self, newRect=None, padding=0.05, lockAspect=None,
                 propagate=True, disableAutoPixel=True) :
        if disableAutoPixel :
    ...
    def scaleToImage(self, image) :
        """Scales such that pixels in image are the same size as screen
        pixels. This may result in a significant performance increase."""
    ...
    def mouseMoveEvent(self, ev) :
        ...
        if self.clickAccepted :  ## Ignore event if an item in the scene has
            #  already claimed it.
            return
        ...
        elif ev.buttons() in [QtCore.Qt.MidButton,
                              QtCore.Qt.LeftButton] :  ## Allow panning by
            # left or mid button.
            px = self.pixelSize()
            tr = -delta * px
    ...
    def pixelSize(self) :
        """Return vector with the length and width of one view pixel in
        scene coordinates"""
        p0 = Point(0, 0)
        p1 = Point(1, 1)
    ...
    def dragEnterEvent(self, ev) :
        ev.ignore()  ## not sure why, but for some reason this class likes
        # to consume drag events
