...
class ColorMapWidget(ptree.ParameterTree) :
    ...
    multiple criteria for assigning colors to each record in a numpy record
    array.
...
class ColorMapParameter(ptree.types.GroupParameter) :
    ...
    def __init__(self) :
        self.fields = {}
        ptree.types.GroupParameter.__init__(self, name='Color Map',
                                            addText='Add Mapping..',
                                            addList=[])
    ...
    def setFields(self, fields) :
        ...
        ============================================================
        Field Options:
        mode           Either 'range' or 'enum' (default is range). For
        'range',
    ...
    def map(self, data, mode='byte') :
        ...
        =================================================================
        **Arguments:**
        data            A numpy record array where the fields in data.dtype
        match those
                        defined by a prior call to setFields().
        mode            Either 'byte' or 'float'. For 'byte', the method
        returns an array
                        of dtype ubyte with values scaled 0-255. For
                        'float', colors are
        ...
        if isinstance(data, dict) :
            data = np.array([tuple(data.values())],
                            dtype=[(k, float) for k in data.keys()])
...
class RangeColorMapItem(ptree.types.SimpleParameter) :
    ...
    def __init__(self, name, opts) :
        ...
        ptree.types.SimpleParameter.__init__(self, name=name,
                                             ...
                                             type='colormap', removable=True,
                                             renamable=True, children=[
                # dict(name="Field", type='list', value=name, values=fields),
                dict(name='Min', type='float', value=0.0, suffix=units,
                     siPrefix=True),
                dict(name='Max', type='float', value=1.0, suffix=units,
                     siPrefix=True),
                dict(name='Operation', type='list', value='Overlay',
                     values=['Overlay', 'Add', 'Multiply', 'Set']),
                dict(name='Channels..', type='group', expanded=False,
                     children=[dict(name='Red', type='bool', value=True),
    ...
    def map(self, data) :
        ...
        scaled = np.clip((data - self['Min']) / (self['Max'] - self['Min']), 0,
                         1)
        cmap = self.value()
        ...
        nanColor = self['NaN']
        nanColor = (
        nanColor.red() / 255., nanColor.green() / 255., nanColor.blue() / 255.,
        nanColor.alpha() / 255.)
...
class EnumColorMapItem(ptree.types.GroupParameter) :
    ...
    def map(self, data) :
        ...
        for v in self.param('Values') :
            ...
            colors[mask] = c
        # scaled = np.clip((data-self['Min']) / (self['Max']-self['Min']),
        # 0, 1)
        # cmap = self.value()
        # colors = cmap.map(scaled, mode='float')
        ...
        # mask = np.isnan(data) | np.isinf(data)
        # nanColor = self['NaN']
        # nanColor = (nanColor.red()/255., nanColor.green()/255.,
        # nanColor.blue()/255., nanColor.alpha()/255.)
        # colors[mask] = nanColor
