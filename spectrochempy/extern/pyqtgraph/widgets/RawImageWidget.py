...
try :
    ...
    # Would prefer `except ImportError` here, but some versions of pyopengl
    # generate
...
class RawImageWidget(QtGui.QWidget) :
    ...
    On some systems this may provide faster video. See the VideoSpeedTest
    example for benchmarking.
    """

    def __init__(self, parent=None, scaled=False) :
        """
        Setting scaled=True will cause the entire image to be displayed
        within the boundaries of the widget. This also greatly reduces the
        speed at which it will draw frames.
        """
...
if HAVE_OPENGL :
    class RawImageGLWidget(QtOpenGL.QGLWidget) :
        ...
        Perfomance varies between platforms; see examples/VideoSpeedTest for
        benchmarking.
        """
        ...
        def uploadTexture(self) :
            ...
            # glTexImage2D(GL_PROXY_TEXTURE_2D, 0, GL_RGBA, shape[0],
            # shape[1], 0, GL_RGBA, GL_UNSIGNED_BYTE, None)
            # if glGetTexLevelParameteriv(GL_PROXY_TEXTURE_2D, 0,
            # GL_TEXTURE_WIDTH) == 0:
            # raise Exception("OpenGL failed to create 2D texture (%dx%d);
            # too large for this hardware." % shape[:2])

            glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, shape[0], shape[1], 0,
