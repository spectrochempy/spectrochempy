...
class TableWidget(QtGui.QTableWidget) :
    """Extends QTableWidget with some useful functions for automatic data
    handling
    and copy / export context menu. Can automatically format and display a
    variety
    of data types (see :func:`setData() <pyqtgraph.TableWidget.setData>` for
    more
    information.
    """
    ...
    def __init__(self, *args, **kwds) :
        ...
        **Keyword Arguments**
        ...
        self._formats = {
            None : None}  # stores per-column formats and entire table format
        self.sortModes = {}  # stores per-column sort mode

        self.itemChanged.connect(self.handleItemChanged)

        self.contextMenu = QtGui.QMenu()
        self.contextMenu.addAction('Copy Selection').triggered.connect(
            self.copySel)
        self.contextMenu.addAction('Copy All').triggered.connect(self.copyAll)
        self.contextMenu.addAction('Save Selection').triggered.connect(
            self.saveSel)
    ...
    def appendData(self, data) :
        ...
        if not self.verticalHeadersSet and header0 is not None :
            labels = [self.verticalHeaderItem(i).text() for i in
                      range(self.rowCount())]
        ...
        if self._sorting and self.horizontalHeader().sortIndicatorSection() \
                >= self.columnCount() :
    ...
    def setFormat(self, format, column=None) :
        ...
        """
        if format is not None and not isinstance(format,
                                                 basestring) and not callable(
                format) :
            raise ValueError(
    ...
    def iteratorFn(self, data) :
        ## Return 1) a function that will provide an iterator for data and
        # 2) a list of header strings
        if isinstance(data, list) or isinstance(data, tuple) :
            return lambda d : d.__iter__(), None
        elif isinstance(data, dict) :
            return lambda d : iter(d.values()), list(
                map(asUnicode, data.keys()))
        elif (hasattr(data, 'implements') and data.implements('MetaArray')) :
            if data.axisHasColumns(0) :
                header = [asUnicode(data.columnName(0, i)) for i in
                          range(data.shape[0])]
    ...
    def serialize(self, useSelection=False) :
        """Convert entire table (or just selected area) into tab-separated
        text values"""
        if useSelection :
            selection = self.selectedRanges()[0]
            rows = list(range(selection.topRow(), selection.bottomRow() + 1))
            columns = list(
                range(selection.leftColumn(), selection.rightColumn() + 1))
        else :
    ...
    def copySel(self) :
        ...
        QtGui.QApplication.clipboard().setText(
            self.serialize(useSelection=True))
    ...
    def keyPressEvent(self, ev) :
        if ev.key() == QtCore.Qt.Key_C and ev.modifiers() == \
...
class TableWidgetItem(QtGui.QTableWidgetItem) :
    ...
    def setFormat(self, fmt) :
        ...
        Added in version 0.9.9.
        """
        if fmt is not None and not isinstance(fmt,
                                              basestring) and not callable(
                fmt) :
            raise ValueError(
                "Format argument must string, callable, or None. (got %s)" %
...
if __name__ == '__main__' :
    ...
    ma = metaarray.MetaArray(np.ones((20, 3)),
                             info=[{'values' : np.linspace(1, 5, 20)}, {
                                 'cols' : [{'name' : 'x'}, {'name' : 'y'},
                                     {'name' : 'z'}, ]}])
    t.setData(ma)
