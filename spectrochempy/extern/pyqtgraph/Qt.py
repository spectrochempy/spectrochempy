...
if QT_LIB is None :
    raise Exception(
        "PyQtGraph requires one of PyQt4, PyQt5 or PySide; none of these "
...
if QT_LIB == PYSIDE :
        ...
        def isQObjectAlive(obj) :
            try :
                if hasattr(obj, 'parent') :
                    ...
                    raise Exception(
                        "Cannot determine whether Qt object %s is still "
                        "alive." % obj)
            except RuntimeError :
                return False
            else :
    ...
    # Credit:
    # http://stackoverflow.com/questions/4442286/python-code-genration-with
    # -pyside-uic/14195313#14195313

    class StringIO(object) :
        """Alternative to built-in StringIO needed to circumvent
        unicode/ascii issues"""
    ...
    VERSION_INFO = 'PyQt4 ' + QtCore.PYQT_VERSION_STR + ' Qt ' + \
                   QtCore.QT_VERSION_STR

elif QT_LIB == PYQT5 :

    # We're using PyQt5 which has a different structure so we're going to
    # use a shim to
    ...
    VERSION_INFO = 'PyQt5 ' + QtCore.PYQT_VERSION_STR + ' Qt ' + \
                   QtCore.QT_VERSION_STR
...
QtVersion = PySide.QtCore.__version__ if QT_LIB == PYSIDE else \
    QtCore.QT_VERSION_STR
