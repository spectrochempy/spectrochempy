...
class LRUCache(object) :
    '''
    This LRU cache should be reasonable for short collections (until around
    100 items), as it does a
    sort on the items if the collection would become too big (so, it is very
    fast for getting and
    setting but when its size would become higher than the max size it does
    one sort based on the
    internal time to decide which items should be removed -- which should be
    Ok if the resizeTo
    isn't too close to the maxSize so that it becomes an operation that
    ...
    if _IS_PY3 :
        ...
        def _resizeTo(self) :
            ordered = sorted(self._dict.values(), key=operator.itemgetter(2))[
                      :self.resizeTo]
            for i in ordered :
                del self._dict[i[0]]
        ...
        def iteritems(self, accessTime=False) :
            ...
            '''
            if accessTime :
                for x in sorted(self._dict.values(),
        ...
        def _resizeTo(self) :
            ordered = sorted(self._dict.itervalues(),
                             key=operator.itemgetter(2))[:self.resizeTo]
            for i in ordered :
                del self._dict[i[0]]
        ...
        def iteritems(self, accessTime=False) :
            ...
            **Arguments**
            ...
            '''
            if accessTime :
                for x in sorted(self._dict.itervalues(),
                                key=operator.itemgetter(2)) :
                    yield x[0], x[1]
            else :
