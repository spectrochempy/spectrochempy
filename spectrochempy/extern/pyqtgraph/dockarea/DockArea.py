...
class DockArea(Container, QtGui.QWidget, DockDrop) :
    ...
    def addDock(self, dock=None, position='bottom', relativeTo=None, **kwds) :
        ...
        =================================================================
        **Arguments:**
        dock           The new Dock object to add. If None, then a new Dock
        will be
                       created.
        position       'bottom', 'top', 'left', 'right', 'above', or 'below'
        relativeTo     If relativeTo is None, then the new Dock is added to
                       fill an
                       entire edge of the window. If relativeTo is another
                       Dock, then
        ...
        neededContainer = \
        {'bottom'   : 'vertical', 'top' : 'vertical', 'left' : 'horizontal',
            'right' : 'horizontal', 'above' : 'tab', 'below' : 'tab'}[position]

        ## Can't insert new containers into a tab container; insert outside
        # instead.
        ...
        if neededContainer != container.type() :
            if neighbor is None :
                container = self.addContainer(neededContainer,
                                              self.topContainer)
            else :
                container = self.addContainer(neededContainer, neighbor)

        ## Insert the new dock before/after its neighbor
        insertPos = {'bottom' : 'after', 'top' : 'before', 'left' : 'before',
            'right'           : 'after', 'above' : 'before',
    ...
    def moveDock(self, dock, position, neighbor) :
        ...
        """
        ## Moving to the edge of a tabbed dock causes a drop outside the tab
        #  box
        if position in ['left', 'right', 'top',
                        'bottom'] and neighbor is not None and \
    ...
    def buildFromState(self, state, docks, root, depth=0) :
        ...
        if typ == 'dock' :
            try :
                ...
                raise Exception(
                    'Cannot restore dock state; no dock with name "%s"' %
                    contents)
        else :
    ...
    def apoptose(self) :
        # print "apoptose area:", self.temporary, self.topContainer,
        # self.topContainer.count()
