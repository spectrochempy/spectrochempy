...
class Dock(QtGui.QWidget, DockDrop) :
    ...
    def __init__(self, name, area=None, size=(10, 10), widget=None,
        ...
        self.moveLabel = True  ## If false, the dock is no longer allowed to
        #  move the label.
    ...
    def updateStyle(self) :
        ...
        # print self.name(), "update style:", self.orientation,
        # self.moveLabel, self.label.isVisible()
        if self.labelHidden :
...
class DockLabel(VerticalLabel) :
    ...
    def mouseMoveEvent(self, ev) :
        if not self.startedDrag and (
            ev.pos() - self.pressPos).manhattanLength() > \
