...
# See: http://stackoverflow.com/questions/16299779/qt-qgraphicsview-unit
# -testing-how-to-keep-the-mouse-in-a-pressed-state
...
def mousePress(widget, pos, button, modifier=None) :
    ...
    if QT_LIB != 'PyQt5' and isinstance(pos, QtCore.QPointF) :
        pos = pos.toPoint()
    event = QtGui.QMouseEvent(QtCore.QEvent.MouseButtonPress, pos, button,
                              QtCore.Qt.NoButton, modifier)
...
def mouseRelease(widget, pos, button, modifier=None) :
    ...
    if QT_LIB != 'PyQt5' and isinstance(pos, QtCore.QPointF) :
        pos = pos.toPoint()
    event = QtGui.QMouseEvent(QtCore.QEvent.MouseButtonRelease, pos, button,
                              QtCore.Qt.NoButton, modifier)
    QtGui.QApplication.sendEvent(widget, event)
...
def mouseMove(widget, pos, buttons=None, modifier=None) :
    ...
    if QT_LIB != 'PyQt5' and isinstance(pos, QtCore.QPointF) :
        pos = pos.toPoint()
    event = QtGui.QMouseEvent(QtCore.QEvent.MouseMove, pos, QtCore.Qt.NoButton,
                              buttons, modifier)
    QtGui.QApplication.sendEvent(widget, event)
