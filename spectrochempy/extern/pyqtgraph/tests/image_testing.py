...
2. Run individual test scripts with the PYQTGRAPH_AUDIT environment variable
set:

       $ PYQTGRAPH_AUDIT=1 python
...
4. Look up the most recent tag name from the `testDataTag` global variable
   ...
   repository:
    ...
    This tag is used to ensure that each pyqtgraph commit is linked to a
    specific
...
def assertImageApproved(image, standardFile, message=None, **kwargs) :
    ...
    ~/.pyqtgraph/test-data. However, it is up to the user to ensure this
    repository
    ...
    if isinstance(image, QtGui.QWidget) :
        ...
        # just to be sure the widget size is correct (new window may be
        # resized):
    ...
    try :
        ...
        if image.shape != stdImage.shape :
            ...
            if (sr[0] != sr[1] or not np.allclose(sr, np.round(sr)) or sr[
                0] < 1) :
        ...
        if os.getenv('PYQTGRAPH_AUDIT_ALL') == '1' :
            raise Exception(
                "Image test passed, but auditing due to PYQTGRAPH_AUDIT_ALL "
...
def assertImageMatch(im1, im2, minCorr=None, pxThreshold=50., pxCount=-1,
                     maxPxDiff=None, avgPxDiff=None, imgDiff=None) :
...
def saveFailedTest(data, expect, filename) :
    ...
    req = urllib.urlencode({'name' : filename, 'data' : base64.b64encode(png)})
...
class ImageTester(QtGui.QWidget) :
    ...
    def __init__(self) :
        ...
        self.views = (
        self.view.addViewBox(row=0, col=0), self.view.addViewBox(row=0, col=1),
    ...
    def test(self, im1, im2, message) :
        ...
        This method displays the test image, reference image, and the
        difference
        between the two. It then blocks until the user selects the test output
        by clicking a pass/fail button or typing p/f. If the user fails the
        test,
        then an exception is raised.
        """
        self.show()
        if im2 is None :
            message += '\nImage1: %s %s   Image2: [no standard]' % (
            im1.shape, im1.dtype)
...
def getTestDataRepo() :
    ...
    if os.path.isdir(dataPath) :
        ...
        print(
            "Attempting to create git clone of test data repo in %s.." %
            dataPath)
...
def transformStr(t) :
    ...
    t.m11(), t.m12(), t.m13(), t.m21(), t.m22(), t.m23(), t.m31(), t.m32(),
    t.m33())
...
class TransposedImageItem(ImageItem) :
    # used for testing image axis order; we can test row-major and col-major
    #  using
