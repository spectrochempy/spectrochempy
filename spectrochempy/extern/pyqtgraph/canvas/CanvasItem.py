...
class CanvasItem(QtCore.QObject) :
    ...
    """CanvasItem takes care of managing an item's state--alpha, visibility, 
    z-value, transformations, etc. and
    ...
    def __init__(self, item, **opts) :
        defOpts = {'name'     : None, 'z' : None, 'movable' : True,
                   'scalable' : False, 'rotatable' : True, 'visible' : True,
                   'parent'   : None}  # 'pos': [0,0], 'scale': [1,1],
        # 'angle':0,
        defOpts.update(opts)
        self.opts = defOpts
        self.selectedAlone = False  ## whether this item is the only one
        # selected
        ...
        ## (It is not safe to use item.setTransform, since the item might
        ...
        self.tempTransform = SRTTransform()  ## holds the additional
        # transform that happens during a move - gets added to the
        # userTransform when move is done.
        self.userTransform = SRTTransform()  ## stores the total transform
        # of the object
    ...
    def setCanvas(self, canvas) :
        ...
        ## It is our responsibility to add all graphicsItems to the canvas's
        #  scene
    ...
    def selectBoxMoved(self) :
        """The selection box has moved; get its transformation information
        and pass to the graphics item"""
        self.userTransform = self.selectBox.getGlobalTransform(
            relativeTo=self.selectBoxBase)
        self.updateTransform()
    ...
    def applyTemporaryTransform(self) :
        ...
        self.selectBoxFromUser()  ## update the selection box to match the
        # new userTransform
    ...
    def resetTemporaryTransform(self) :
        self.tempTransform = SRTTransform()  ## don't use Transform.reset(
        # )--this transform might be used elsewhere.
        self.updateTransform()
    ...
    def updateTransform(self) :
        """Regenerate the item position from the base, user, and temp
        transforms"""
        transform = self.baseTransform * self.userTransform * \
                    self.tempTransform  ## order is important
    ...
    def displayTransform(self, transform) :
        ...
        self.transformGui.translateLabel.setText(
            "Translate: (%f, %f)" % (tr['pos'][0], tr['pos'][1]))
        self.transformGui.rotateLabel.setText(
            "Rotate: %f degrees" % tr['angle'])
        self.transformGui.scaleLabel.setText(
            "Scale: (%f, %f)" % (tr['scale'][0], tr['scale'][1]))
    ...
    def selectBoxToItem(self) :
        """Move/scale the selection box so it fits the item's bounding rect.
        (assumes item is not rotated)"""
    ...
    def showSelectBox(self) :
        """Display the selection box around this item if it is selected and
        movable"""
        if self.selectedAlone and self.isMovable() and self.isVisible() :  #
            #  and len(self.canvas.itemList.selectedItems())==1:
            self.selectBox.show()
        else :
            self.selectBox.hide()
