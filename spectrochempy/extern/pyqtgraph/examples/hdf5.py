# -*- coding: utf-8 -*-
"""
In this example we create a subclass of PlotCurveItem for displaying a very
large
...
reads only the portion of the HDF5 data that is necessary to display the
visible
portion of the data. This is further downsampled to reduce the number of
samples
being displayed.
    ...
    def updateHDF5Plot(self) :
        ...
        if ds == 1 :
            ...
            while sourcePtr < stop - 1 :
                ...
                #  shape
                visible[
                targetPtr :targetPtr + chunk.shape[0] * 2 :2] = chunkMin
                visible[1 + targetPtr :1 + targetPtr + chunk.shape[
                                                           0] * 2 :2] = \
                    chunkMax
...
if len(sys.argv) > 1 :
    ...
    if not os.path.isfile(fileName) :
        size, ok = QtGui.QInputDialog.getDouble(None, "Create HDF5 Dataset?",
                                                "This demo requires a large "
                                                "HDF5 array. To generate a "
                                                "file, enter the array size "
                                                "(in GB) and press OK.",
                                                2.0)
        if not ok :
            sys.exit(0)
            ...
            # raise Exception("No suitable HDF5 file found. Use createFile()
