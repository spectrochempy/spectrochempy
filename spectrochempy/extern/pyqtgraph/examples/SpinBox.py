...
spins = [("Floating-point spin box, min=0, no maximum.",
...
pg.SpinBox(value=1.0, suffix='V', siPrefix=True, dec=True, step=0.5,
           minStep=0.01)), (
"Float with SI-prefixed units,<br>dec step=1.0, minStep=0.001",
pg.SpinBox(value=1.0, suffix='V', siPrefix=True, dec=True, step=1.0,
           minStep=0.001)), ]
...
changedLabel = QtGui.QLabel()  ## updated only when editing is finished or
# mouse wheel has stopped for 0.3sec
...
# t1 = QtGui.QLineEdit()
# g.addRow(s1, t1)
# s2 = SpinBox(value=10e-6, dec=True, step=0.1, minStep=1e-6, suffix='A',
# siPrefix=True)
# t2 = QtGui.QLineEdit()
# g.addRow(s2, t2)
# s3 = SpinBox(value=1000, dec=True, step=0.5, minStep=1e-6, bounds=[1,
# 1e9], suffix='Hz', siPrefix=True)
# t3 = QtGui.QLineEdit()
# g.addRow(s3, t3)
...
##QtCore.QObject.connect(sb, QtCore.SIGNAL('valueChanged(double)'), lambda
# v: sys.stdout.write(str(sb) + " valueChanged\n"))
##QtCore.QObject.connect(sb, QtCore.SIGNAL('editingFinished()'), lambda:
# sys.stdout.write(str(sb) + " editingFinished\n"))
# sb.sigValueChanged.connect(valueChanged)
# sb.sigValueChanging.connect(valueChanging)
# sb.editingFinished.connect(lambda: sys.stdout.write(str(sb) + "
# editingFinished\n"))
