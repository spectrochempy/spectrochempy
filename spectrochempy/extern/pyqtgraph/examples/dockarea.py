# -*- coding: utf-8 -*-
"""
This example demonstrates the use of spectrochempy.extern.pyqtgraph's dock widget system.

The dockarea system allows the design of user interfaces which can be
rearranged by
the user at runtime. Docks can be moved, resized, stacked, and torn out of
the main
...
programatically generate complex dock arrangements). Additionally,
Qt's docks are
...
import spectrochempy.extern.pyqtgraph.console
import numpy as np

from spectrochempy.extern.pyqtgraph.dockarea import *
...
area.addDock(d1,
             'left')  ## place d1 at left edge of dock area (it will fill
# the whole space since there are no other docks yet)
...
the borders between docks may be dragged to resize. Docks that are dragged 
on top
