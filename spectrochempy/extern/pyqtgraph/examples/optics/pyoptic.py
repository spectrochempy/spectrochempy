# -*- coding: utf-8 -*-
import spectrochempy.extern.pyqtgraph as pg
from spectrochempy.extern.pyqtgraph.Qt import QtGui, QtCore
import numpy as np
import csv, gzip, os
from spectrochempy.extern.pyqtgraph import Point
...
class GlassDB :
    ...
    def __init__(self, fileName='schott_glasses.csv') :
        ...
        for l in lines[1 :] :
            ...
            for i in range(1, len(l)) :
                info[header[i]] = l[i]
            self.data[l[0]] = info
        self.data['Corning7980'] = {
        ## Thorlabs UV fused silica--not in schott catalog.
            'B1'          : 0.68374049400, 'B2' : 0.42032361300,
            'B3'          : 0.58502748000, 'C1' : 0.00460352869,
            'C2'          : 0.01339688560, 'C3' : 64.49327320000,
            'TAUI25/250'  : 0.95,
        ## transmission data is fabricated, but close.
            'TAUI25/1400' : 0.98, }
    ...
    def ior(self, glass, wl) :
        ...
        if wl not in cache :
            ...
            w2 = (wl / 1000.) ** 2
            n = np.sqrt(
                1.0 + (B[0] * w2 / (w2 - C[0])) + (B[1] * w2 / (w2 - C[1])) + (
                B[2] * w2 / (w2 - C[2])))
            cache[wl] = n
        return cache[wl]
...
class ParamObj(object) :
    ...
    def setParams(self, **params) :
        """Set parameters for this optic. This is a good function to
    ...
    def __getitem__(self, item) :
        # bug in pyside 1.2.2 causes getitem to be called inside
        # QGraphicsObject.parentItem:
        return self.getParam(
            item)  # PySide bug: https://bugreports.qt.io/browse/PYSIDE-441
...
class Optic(pg.GraphicsObject, ParamObj) :
    ...
    def roiChanged(self, *args) :
        ...
        self.setParams(angle=self.roi.angle(),
                       pos=pos + (self.gitem.pos() - o1))
...
class Lens(Optic) :
    ...
    def propagateRay(self, ray) :
        """Refract, reflect, absorb, and/or scatter ray. This function may
        create and return new rays"""

        """
        NOTE:: We can probably use this to compute refractions faster: (from 
        GLSL 120 docs)
        ...
        for i in [0, 1] :
            ...
            # trans = self.sceneTransform().inverted()[0] *
            # surface.sceneTransform()
            # p1 = trans.map(p1)
            if p1 is None :
                ray.setEnd(None)
                break
...
class CircularSolid(pg.GraphicsObject, ParamObj) :
    ...
    def __init__(self, pen=None, brush=None, **opts) :
        ...
        ParamObj.__init__(self)
        self.surfaces = [CircleSurface(defaults['r1'], defaults['d1']),
                         CircleSurface(-defaults['r2'], defaults['d2'])]
    ...
    def updateSurfaces(self) :
        ...
        self.path.connectPath(
            self.surfaces[0].path.translated(self.surfaces[0].pos()))
        self.path.connectPath(self.surfaces[1].path.translated(
            self.surfaces[1].pos()).toReversed())
...
class CircleSurface(pg.GraphicsObject) :
    def __init__(self, radius=None, diameter=None) :
        ...
        radius is the radius of the surface. If radius is None, the surface
        is flat.
...
class Ray(pg.GraphicsObject, ParamObj) :
    ...
    def __init__(self, **params) :
        ParamObj.__init__(self)
        defaults = {'ior' : 1.0, 'wl' : 500, 'end' : None,
            'dir'         : Point(1, 0), }
        self.params = {}
        pg.GraphicsObject.__init__(self)
        self.children = []
