## make this version of spectrochempy.extern.pyqtgraph importable before any others
...
if not hasattr(sys, 'frozen'):
    ...
    if 'pyqtgraph' in os.listdir(path):
        sys.path.insert(0, path) ## examples adjacent to spectrochempy.extern.pyqtgraph (as in source tree)
    else:
        for p in sys.path:
            if len(p) < 3:
                continue
