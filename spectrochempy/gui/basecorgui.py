...
from ..core.api import CoordRange
    ...
    * Subplot to display the result
    ...
    class BlGui(tk.Tk):
        ...
        def computebaseline(self, X, r):
            ...
            interp = self.interpolation.get(
                self.interpolation.curselection()).lower()
        ...
        def initialize(self, X):
            ...
            b1 = tk.Button(self, text="Correct spectra",
                           command=lambda: self.computebaseline(X, self.r))
            b1.pack()

            b2 = tk.Button(self, text="Save & Exit",
                           command=lambda: self.closebl())
            b2.pack()
            ...
            def onselect(xmin,
                         xmax):  # add the selected range to the baseline range
                ...
                plt.subplot(211)
                # update rectangles 
                for patch in [patch for patch in ax1.patches if
                              type(patch) == mpl.patches.Polygon]:
                    patch.remove()
                self.p = []
                for x in self.r:  # plot the new ones
                    self.p.append(
                        plt.axvspan(x[0], x[1], ymin=0, ymax=1, facecolor='g',
                                    alpha=0.3))
            ...
            def on_press(event):  # removes a range on right click
                ...
                if event.button == 3:
                    for xrange in self.r:
                        if (event.xdata >= xrange[0]) and (
                            event.xdata <= xrange[1]):
                            ...
                            for x in self.r:  # plot the new ones
                                self.p.append(
                                    plt.axvspan(x[0], x[1], ymin=0, ymax=1,
            ...
            ax1 = plt.subplot(211)
            ax1.set_title(
                'Baseline select: press left & drag to select / right click to delete')
