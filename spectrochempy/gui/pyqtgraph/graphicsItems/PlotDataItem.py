...
class PlotDataItem(GraphicsObject):
    ...
    **Signals:**
    ...
    ###### Interface to matplotlib specific commands ################

    def set_color(self, color):
        self.setPen(color)

    def set_linewidth(self, lw):
        self.setPen(width=lw)
        ...
        if (hasattr(obj, 'implements') and obj.implements('NDDataset')):
            ...
            if obj.ndim == 1:
                if obj.dtype.names is None:
                    ...
                    return 'recarray'
            elif obj.ndim == 2 and obj.dtype.names is None \
                    and obj.shape[1] == 2:
                return 'Nx2array'
            else:
                raise Exception('array shape must be (N,) or (N,2); got %s '
                                'instead' % str(obj.shape))
