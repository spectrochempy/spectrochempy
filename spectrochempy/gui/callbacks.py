...
class Callbacks(object):
    ...
    def dataset_list(self, *datasets):
        return [
            dbc.ListGroupItem(
                [
                    dbc.Checkbox(
                        id=f"dataset-checkbox-{i}",
                        style={"float": "right"},
                        checked=True,
                    ),
                    *self.dataset_itemlist(ds),
                ]
            )
    ...
    def add_callbacks(self, app):
        ...
        @app.callback(
            ...
            [State("original-data", "data"), State("intermediate-data", "data")],
        )
        ...
        def tab_display_control(children):
            if not children:
                return dict({"display": "none"}), True, True, True
            else:
                return dict({"display": "block"}), False, False, False
        ...
        @app.callback(
            ...
            [Input(f"project-new", "n_clicks"), Input(f"project-open", "n_clicks")],
        )
