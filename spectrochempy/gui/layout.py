    ...
    return dbc.Card(
        [
            ...
            dbc.Collapse(
                id=f"open-{key}-more",
                is_open=True,
                children=card_body(children=body, id=id, className=className),
            ),
        ]
    )
...
def generic_card(key, title, markdown="", children=None):
    return collapsable_card(
        ...
        body=[
            dcc.Markdown(children=markdown, className="Markdown"),
            html.Div(children, id=key),
        ],
        key=key,
    )
...
class Layout(object):
    def __init__(
        self, calling_script_name="", fix_navbar=False, data_storage="session"
    ):
        self.LOGO = html.Img(
            src="data:image/png;base64,{}".format(
                base64.b64encode(
                    open(
                        os.path.join(
                            os.path.dirname(os.path.abspath(self.calling_script_name)),
                            "assets",
                            "scpy_logo.png",
                        ),
                        "rb",
        ...
        self.TITLE = dbc.NavbarBrand(
            children=dcc.Markdown("**S**pectro**C**hem**P**y *by Dash*"),
            id="navbarbrand",
            className="navbarbrand",
        )
        self.data_storage = data_storage
        self.STORE = [
            dcc.Store(id="original-data", storage_type=self.data_storage),
                        ...
                        [
                            dbc.InputGroupAddon(
                                dbc.DropdownMenu(
                                    ...
                                    addon_type="prepend",
                                    children=[
                                        dbc.DropdownMenuItem("New", id="project-new"),
                                        dbc.DropdownMenuItem(
                                            dcc.Upload("Open", id="upload-project"),
                                            id="project-open",
                                        ),
                                        dbc.DropdownMenuItem(
                                            "Save", id="project-save", disabled=True
                                        ),
                                        dbc.DropdownMenuItem(
                                            "Close", id="project-close", disabled=True
                                        ),
                                    ],
                                )
                            ),
...
If an offset is defined then the actual roi below is displayed accordingly (*see actual roi values below*).            
            ...
            dbc.InputGroup(
                [
                    dbc.InputGroupAddon("Offset", addon_type="prepend"),
                    numeric_input(id="y-offset", placeholder="Offset value, default=0"),
                ],
                size="sm",
            ),
...
masks.
    ...
    def subtraction(self):
        return [
            ...
            dcc.Dropdown(
                options=[
                    {"label": "Subtract first row", "value": "first"},
                    {"label": "Subtract last row", "value": "last"},
                    {
        ...
        return dbc.Card(
            dbc.CardBody(
                [
                    generic_card(
                        "subtraction",
                        "Subtraction correction",  # DO NOT FORGET TO ADD NEW KEYS IN CALLBACK
                        children=self.subtraction(),
                    ),
                    generic_card(
                        "baseline", "Baseline correction", children=self.baseline()
                    ),
                    generic_card(
                        "peakpicking",
                        "Peak Picking",
            ...
            [
                dbc.Tabs(
                    [
                        ...
                        dbc.Tab(
                            self.data_tab(), label="Data", tab_id="data", id="data-tab"
                        ),
                        dbc.Tab(
                            self.graph_tab(),
                            label="Graph",
        ...
        layout = html.Div(
            [
                ...
                dbc.Container(
                    children=[
                        dbc.Row(
                            [self.tabs_zone(), self.graph_zone()],
                        ),
                    ],
                    fluid=True,
