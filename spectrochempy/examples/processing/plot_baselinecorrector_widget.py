...
In this example, we perform interactively a baseline correction of a 2D dataset.
"""
...
class BaselineCorrector_old:
    ...
    def __init__(self, X=None, initial_ranges=None):
        ...
        # Parameters widgets
        self._multivariate_radioButtons = widgets.RadioButtons(
            options=[False, "svd", "nmf"], value=False, description="Multivariate"
        )

        self._n_components_slider = widgets.IntSlider(
            description="N components", value=3, min=1, max=8, disabled=False
        )
        ...
        self._model_dropdown = widgets.Dropdown(
            description="Model",
            options=["abc", "polynomial", "detrend", "asls", "snip"],
            value="abc",
        )
...
# %%
class BaselineCorrector(tr.Hastraits):
    """
    Launch a GUI for baseline corrections.
...
# ------------------------------------------
# To perform baseline correction interactively, one can use the BaselineCorrector widget.
...
out = BaselineCorrector(X)  # scp.  < TODO: chnag back
