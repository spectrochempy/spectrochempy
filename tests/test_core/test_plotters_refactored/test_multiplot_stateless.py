...
class TestMultiplotStateless:
    ...
    def test_basic_grid_layout(self, sample_1d_dataset):
        ...
        assert all(isinstance(ax, plt.Axes) for ax in axes), "All elements should be Axes objects"
        ...
        for i, (orig_state, dataset) in enumerate(zip(original_states, datasets)):
    ...
    def test_multiplot_single_dataset(self, sample_1d_dataset):
        ...
        assert isinstance(axes, plt.Axes), "Single dataset multiplot should return single Axes"
        
        # Compare with direct plot
        ax_direct = sample_1d_dataset.plot()
        
        # Should have similar basic properties (both should be line plots)
        assert len(axes.get_lines()) == len(ax_direct.get_lines()), "Both should have same number of line objects"
    ...
    def test_multiplot_method_selection(self, sample_1d_dataset, sample_2d_dataset):
        ...
        assert len(axes[1].collections) > 0, "Map method should create contour collections"
