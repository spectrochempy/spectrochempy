...
class TestPhase1NoGlobalMutation:
    ...
    def test_no_global_rcparams_mutation_after_import(self):
        ...
        try:
            ...
            for key in initial_rcparams:
                if key in current_rcparams and current_rcparams[key] == initial_rcparams[key]:
                    unchanged_keys += 1
    ...
    def test_preferences_change_no_global_mutation(self):
        ...
        assert "lines.linewidth" not in rcparams_after or rcparams_after["lines.linewidth"] != 3.0, (
    ...
    def test_style_context_isolation(self):
        ...
        with plt.style.context("default"):
            ...
            assert "default" in str(context_params) or "axes.prop_cycle" in context_params
        ...
        for key in rcparams_before:
            if rcparams_before[key] != rcparams_after.get(key):
                if key in ["text.usetex", "mathtext.fontset"]:  # These may legitimately change
                    continue
        ...
        if "lines.linewidth" in rcparams_before:
            assert rcparams_after["lines.linewidth"] == rcparams_before["lines.linewidth"]