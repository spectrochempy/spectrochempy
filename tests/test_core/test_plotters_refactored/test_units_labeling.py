...
class TestUnitsLabeling:
    ...
    def test_unitless_coordinates(self):
        ...
        assert "/dimensionless" not in xlabel, "Should not contain unit suffix for unitless"
        assert "/" not in xlabel.split()[-1] if xlabel else False, "Should not have trailing slash for unitless"
