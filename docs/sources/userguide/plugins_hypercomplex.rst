...
Hypercomplex data requires the plugin, which registers:

* a dataset accessor ``dataset.hyper``;
* numeric-backend hooks so that generic math operations
  (``absolute``, ``conjugate``, ``max``, ``min``, …) work correctly
  on quaternion arrays;
* an ``fft.encoding`` handler so that 2D NMR acquisition encodings
  (STATES, TPPI, ECHO-ANTIECHO) produce quaternion spectra.
