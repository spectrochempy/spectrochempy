...
- Reexported scikit-learn's ``KFold``, ``GroupKFold``, and ``LeaveOneOut`` as
  ``scp.KFold``, ``scp.GroupKFold``, and ``scp.LeaveOneOut`` so supported
  validation protocols can be constructed from the SpectroChemPy namespace.
