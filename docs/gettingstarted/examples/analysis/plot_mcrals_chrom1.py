...
For the user convenience, this dataset is present in the `datadir` of SpectroChemPy
as 'als2004dataset.MAT' .
...
print("NDDataset names: " + str([ds.name for ds in datasets]))

# %%
# We are interested in the last dataset ('m1') that contains a single HPLS-DAD run
# (51x96)  dataset. As usual, the 51 rows correspond to the 'time axis' of the HPLC run,
# and the 96 columns to the 'wavelength' axis
# of the UV spectra. The original dataset does not contain information as to the actual
# time and wavelength coordinates.
#
...
# %%
# The optimization has converged. We can get the concentration (C) and pure spectra
# profiles (St) and plot them
