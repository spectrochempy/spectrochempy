...
# expected to have read the [Import](../IO/import.ipynb), [Import IR](../IO/importIR.ipynb),  [slicing](../processing/slicing.ipynb) tutorials. 
#
...
#
# The use of the "magic command" `%matplotlib widget` in a notebook triggers the plotting of interactive plots integrated in the notebook with basic tools to navigate inside the plot. As shown below from top to bottom of the side bar:
# - hide/show the tools, 
# - reset view ('home'), 
...
# - move ('arrow cross') 
# - zoom ('rectangle'), 
# - save image ('floppy disc'). Another possibility is to use `%matplotlib qt` instead. In this case, the plot is generated 
#
# In this interactive mode, the current abscissa and ordinates are inidated and when the mous pointer is displace in the plot area. 
...
# %% [markdown]
# Once a given maximum has been approximately located manually with the mouse, it is possible to obtain exactly the actual maximum using the slicing. For instance, after zooming on the highest peak of the last spectrum, one finds that it is locate at ~ 2115.5 cm$^{-1}$. The exact coordinate can the be obtained using the following: (see the [slicing tutorial](../processing/slicing.ipynb) for more info):

# %%
X.x[2115.5].data[0]   # X.x[2115.5] returns a Coord object of 1 element,  
                      # which value is accessed through the data attribute (a ndarray), which value is
                      # in the first (an only) element indexed '0' 
...
# %% [markdown]
# ### Note: on Magics
# Technically, a "magic command", invoked by the % sign, controls the behaviour of IPython itself.  In the present case (`%matplotlib widget`), a particular matplotlib backend is triggered *once and for all in the current ipython session*. The only way to change it back to the previous one (`%matplotlib inline` which is the default) requires restarting the Ipython Kernel in the menu of Jupyter. 
#
# Another useful "magic" for interactive plots is `%matplotlib qt` (also definitive in the current session), which will generate interactive plots in separate and independant windows, which is usually faster and more fluid, in particular for complex plots.   
...
for peak in peaks:  #loop over peaks
    plt.plot(peaks.x, peaks.data.T, 'v', color='black')   # the data field must be transposed for plot; 'v' is the triangle_down marker
...
# %% [markdown]
# It should be noted that this method finds only true maxima, not shoulders (!). For the detection of such undrelying peaks,the use of methods based on derivatives or advanced detection methods - which will be treated in separate tutorial - are required.
# Once ther maxima of a given peak have been found, it is possible, for instance, to plot its evolution with, e.g. the time. For instance for the peaks located at 2220-2180 cm$^{-1}$:
...
# - `height`: minimal required height of the peaks (single number) or minimal and maximal heights (sequence of two numbers)
# - `prominence`: minimal prominence of the peak to be detected (single number) or minimal and maximal prominence (sequence of 2 numbers). In brief the "prominence" of a peak measures how much a peak stands out from its surrounding and is the vertical distance between the peak and its lowest "contour line". It should not be confused with the height as a peak can have an important height but a small prominence when surronded by other peaks (see below for an illustration). 
#     - in addition to the prominence, the user can define `wlen`, the width (in points) of the window used to look at neighboring minima, the peak maximum being is at the center of the window.
...
# - `distance`: the required minimal horizontal distance (in points, >= 1) between neighbouring peaks. Smaller peaks are removed first.
# - `width`: Required minimal width of peaks in samples (single number, in points) or minimal and maximal width. The width is assessed from the peak height, prominence and neighboring signal.  
#     - In addition the user can define `rel_height` (a float between 0. and 1. used to compute the width - see the [scipy documentation](https://docs.scipy.org/doc/scipy/reference/generated/scipy.signal.peak_widths.html)  for further details.
#     
# Finally we mention en passant a last parameter, `plateau_size()` used for selecting peaks having truly flat tops (as in e.g. square-boxed signals). 
#
...
# default settings
peaks, properties = s.find_peaks() ; s.plot()   # 2 instructions on one line are separated by ";"
for peak in peaks: plt.plot(peaks.x, peaks.data.T, 'v', color='black')  # a single instruction-for loop can be put in 1 line.

# find peaks heights than 0.05  (NB: the spectra are shifted for the display. Refer to the 1st spectrum for true heights)    
peaks, properties = s.find_peaks(height=0.05) ; (s + 0.05).plot(clear=False) ; 
for peak in peaks: plt.plot(peaks.x, peaks.data.T + 0.05, 'v', color='blue')   

# find peaks heights between 0.05 and 0.2 (the highest peak won't be detected)     
peaks, properties = s.find_peaks(height=(0.05, 0.2)) ;  (s + 0.1).plot(clear=False) ; 
for peak in peaks: plt.plot(peaks.x, peaks.data.T + 0.1, 'v', color='red')   
    
# find peaks with prominence >= 0.05 (only the two most prominent peaks are detected)
peaks, properties = s.find_peaks(prominence=0.05); (s + 0.15).plot(clear=False) 
for peak in peaks: plt.plot(peaks.x, peaks.data.T + 0.15, 'v', color='purple')  
    
# find peaks with distance >= 10 (only tht highest of the two maxima at ~ 2075 is detected)
peaks, properties = s.find_peaks(distance=10); (s + 0.20).plot(clear=False)
for peak in peaks: plt.plot(peaks.x, peaks.data.T + 0.20, 'v', color='green')   
    
# find peaks with width >= 10 (none of the two maxima at ~ 2075 is detected)
peaks, properties = s.find_peaks(width=10); (s + 0.25).plot(clear=False)
for peak in peaks: plt.plot(peaks.x, peaks.data.T + 0.25, 'v', color='grey')   
...
# %% [markdown]
# The actual prominence of the peak is this 0.0689, a value significantly lower that is peak height (0.1995), as could be expected by the illustration above. The algorithm used to determine de 'bases' is illustrated below:
# - (1) extend a line to the left and right of the maximum until until it reaches the window border (here on the left) or the signal (here on the right.
# - (2) find the minimum value within the intervals defined above. These points are the peak's bases.
# - (3) use the higher base (here the right base) and peak maximum to calculate the prominence. 
#
#
...
# %% [markdown]
# Finally, we illustrate how the use of the `wlen` parameter - which limits the search of the "base points" in a window of `wlen` points - affects the prominence returned for the largest peak. As illustrated below, it will reduce the prominence of the peak:
#
# <img src="figures/prominence_wlen.png" alt="prominence_def" width="700" align="center" />
#
# This impact can be checked with the code below
...
# %% [markdown]
# In practice, the `wlen` parameter can thus by very useful as it can be used to select peaks with given relative heights to their neighbours.  

# %% [markdown]
# #### Width

# %% [markdown]
# The peak widths, as returned by `find_peaks()` can be *very approximate* and for precise assessement, more advanced methods (such as peak fitting, also implemented in spectrochempy - see [this example](https://www.spectrochempy.fr/dev/gallery/auto_examples/fitting/plot_fit.html#sphx-glr-gallery-auto-examples-fitting-plot-fit-py) should be used. On the other hand, the magnitude of the widths are generally fine. 
#
# Their estimates is based on an alghorithm similar to that used for the "bases" above, except that the horizontal line starts from a `width_height` computed from the peak height subtracted by a *fraction* of the peak prominence defined bay `rel_height` (default = 0.5). The algorithm is ilustrated below for the two most prominent peaks:     
#
# <img src="figures/width_algo.jpg" alt="width_algo" width="900" align="center" />
#
# When the `width` keyword is used, `properties` disctionarry returns the prominence parameters (as it is used for the calculation of the width), the width and the left and right interpolated positions ("ips") of the intersection of the horizontal line with the spectrum:
...
# %% [markdown]
# The code below shows how these heights and widths can be extracted from the disctiopnary and plotted using the matplotlib library:
...
# %% [markdown]
# As stressed above, we see here that the peak width is very approximate and probably exaggerated in the present case due to the presence of the second peak on the left. Here a better estimate would be obtained vby considering the right half-width or reducing the `rel_height` parametr as shown beow.

# %% [markdown]
# ### 2.4 A code snippet to display properties
#
# The self-contained code snippet below can be used to display in a matplolib plot and print the various peakproperties of a single peak as returned by `find_peaks()`: 
