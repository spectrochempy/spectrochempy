...
#
#
# The previous correction was made using the default parameters for the interpolation ,i.e. an interpolation using cubic Hermite spline interpolation: 
# `interpolation='pchip'` (`pchip` stands for **P**iecewise **C**ubic **H**ermite **I**nterpolating **P**olynomial). This option triggers the use of [scipy.interpolate.PchipInterpolator()](https://docs.scipy.org/doc/scipy/reference/generated/scipy.interpolate.PchipInterpolator.html) to which we refer the interested readers.
# The other interpolation method is the classical polynomial interpolation (`interpolation='polynomial'`) in which case the order can also be set (e.g. `order=3`, the default value being 6). In this case, the base methods used for the interpolation are those of the [polynomial module](https://numpy.org/doc/stable/reference/routines.polynomials.polynomial.html) of spectrochempy, in particular the [polyfit()](https://numpy.org/doc/stable/reference/generated/numpy.polynomial.polynomial.polyfit.html#numpy.polynomial.polynomial.polyfit) method. 
#
# For instance:
...
subplot6 = blc.corrected.plot()  # the 'corrected' attribute contains the corrected dataset                                               
...
NDDataset.plot() ; plt.gca().get_xaxis().set_visible(False)   # plot original NDDatase and remove xaxis info
blc.corrected.plot()
