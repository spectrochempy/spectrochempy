...
#
# - `name` is the name of the group of spectra as it appears in the .spg file.
# OMNIC sets this name to the .spg
# filename used at the creation of the group. In this example, the name
# ("Group sust Mo_Al2O3_base line.SPG") differs
...
# It can be accessed (or even
# changed) using `X.created`.
#
# - `comment` indicates the complete pathname of the .spg file. As the pathname is also
# given in the history (below), it can be a good practice to give a self explaining
# description of the group, for instance:
...
#
# - `title` (not to be confused with the `name` of the dataset) describes the nature of
# data (here **absorbance**).
#
...
# %% [markdown]
# - `dims`: Note that the `x` and `y` dimensions are the second and first dimension
# respectively. Hence, `X[i,j]`
# will return
# the absorbance of the ith spectrum at the jth  wavenumber. However, this is subject
# to change, for instance if you
# perform operation on your data such as [Transposition]
# (../processing/transformations.ipynb#Transposition). At any time
# the attribute `dims` gives the correct names (which can be modified) and order of the
# dimensions.
...
# As such these may be not convenient to use directly (they are currently in the
...
# %% [markdown]
# It is also possible to use the ability of SpectroChemPy to handle unit changes.
# For this one can use the `to` or
# `ito` (
# inplace) methods.
# ```ipython3
...
# %% [markdown]
# Finally, if for instance you want the `x` time axis to be shifted by 2 minutes,
# it is also very easy to do so:
...
# (e.g. different spacings or wavenumber
...
# %% [markdown]
# The omnic reader can also import several spa files together, providing that they
# share a common axis for the
...
# %% [markdown]
# In such a case ase these .spa files are alone in the directory, a very convenient
# is to call the read_omnic method
...
# <b>Warning</b>
#
# There is a difference in specifying the directory to read as an argument as above
# or as a keyword like here:
# ```ipython3
...
# [brukeropusreader](https://github.com/spectrochempy/brukeropusreader) developed
# by QED. It imports absorbance
...
#
# [JCAMP-DX](http://www.jcamp-dx.org/) is an open format initially developed for
# IR data and extended to
# other spectroscopies. At present, the JCAMP-DX reader implemented in Spectrochempy
# is limited to IR data and
...
# %% [markdown]
# It is important to note here that the conversion to JCAMP-DX changes the last
# digits of absorbance and wavenumbers:
