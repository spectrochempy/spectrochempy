...
The `NDDataset` object is the main object used by `SpectroChemPy` .

Like numpy `~numpy.ndarrays`, `NDDataset` have the capability to be sliced, sorted and subjected to mathematical operations.
...
or other `NDDataset` s, etc.).

This offers a lot of flexibility in using `NDDataset` s that, we hope, will be useful for the users.
