...
class NDPanel():
    pass  # fake waiting for a NDPanel class


class SCPYArrayInterface(GridInterface):
    types = ()
    ...
    @classmethod
    def loaded(cls):
        return 'spectrochempy' in sys.modules
    ...
    def applies(cls, obj):
        ...
        return isinstance(obj, (NDDataset))  # , NDPanel))
    ...
    def init(cls, eltype, data, kdims, vdims):
        ...
        if not isinstance(data, NDPanel):
            if kdims is None:
            ...
            for vdim in vdims:
                arr = data[vdim.name]
                if not isinstance(arr, NDDataset):
                    arr = NDDataset(arr, coords=coords, **xr_kwargs)
        ...
        not_found = []
        for d in kdims:
            if not any(d.name == k or (isinstance(v, NDDataset) and d.name in v.dims)
    ...
    def coords(cls, dataset, dimension, ordered=False, expanded=False, edges=False):
        ...
        if irregular or expanded:
            ...
            return data.values if isinstance(data, NDDataset) else data
    ...
    def concat_dim(cls, datasets, dim, vdims):
        ...
        return NDDataset.concatenate([ds.assign_coords(**{dim.name: c}) for c, ds in datasets.items()],
                                     dim=dim.name)
    ...
    def add_dimension(cls, dataset, dimension, dim_pos, values, vdim):
        ...
        arr = NDDataset(values, coords=coords, name=dim,
                        dims=tuple(d.name for d in dataset.kdims[::-1]))
        return dataset.data.assign(**{dim: arr})


Interface.register(SCPYArrayInterface)
