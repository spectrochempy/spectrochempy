...
# `roi` Semantic Contract

## Status

Implemented maintainer RFC.

This document now serves as the maintained decision record for the historical
`roi` field.

## Implementation Status

`NDDataset.roi` has been assessed as orphaned historical state and should be
removed from the public runtime data model. Legacy serialized `roi` / `_roi`
entries should remain load-compatible and be ignored on read.
