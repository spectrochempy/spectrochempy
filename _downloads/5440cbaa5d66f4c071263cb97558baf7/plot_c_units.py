...
# `pint <https://pint.readthedocs.io>`_ to define and perform operations on
# data with units.
...
# %%
# * ``wavenumbers`` (``x`` ) coordinates are here expressed in :math:`\mathrm{cm}^{-1}`
# * and ``data`` are in absorbance (:math:`\mathrm{a.u.}`) units.
...
# :math:`\mathrm{cm}^{-1}` that can be converted to
# :math:`\mathrm{THz}`, for instance:
...
# as SpectroChemPy, thanks to `pint <https://pint.readthedocs.io>`_, knows how
